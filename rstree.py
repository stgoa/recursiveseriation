...
class RecursiveSeriation:
	...
	def ineq(self, A,A_prime,B, B_prime, z):
		return max(np.min([self.diss(z,a) for a in A]),np.min([self.diss(z,a_prime) for a_prime in A_prime])) < min(np.max([self.diss(z,b) for b in B]),np.max([self.diss(z,b_prime) for b_prime in B_prime]))
	...
	def internal_orientation(self, tree):
		if len(tree.children) >2 and tree.depth > 1:
			...
			while not all([tree.children[i].singleton for i in range(1,len(tree.children)-1)]):
				...
				for i in range(1,len(tree.children)-1):
					...
					if not T_i.singleton:
						...
						orientation = self.border_candidates_orientation(A_prime,A, B, B_prime)
	...
	def final_internal_orientation(self, tree):
		...
		while not all([tree.children[i].singleton for i in range(len(tree.children))]):
			if self.verbose > 3:
			...
			if len(tree.children) == 2:
				...
				if not T_i.singleton:
					...
					if self.verbose > 3:
						...
						print("break condition", [(tree.children[i], tree.children[i].singleton) for i in range(len(tree.children))])
				...
				for i in range(len(tree.children)):
					...
					if not T_i.singleton:
						...
						A_prime = tree.children[(i-1)%len(tree.children)].borders()
						A = T_i.left_borders()
						B = T_i.right_borders()
						B_prime = tree.children[(i+1)%len(tree.children)].borders()
	...
	def sort(self, trees = None, iter = 0):
		...
		if self.memory_save:
			def dminw(x,y):
				val, _ = self.dmin(x,y)
				return val
			G = NNGraph(node_list = trees, dissimilarity = dminw, verbose = self.verbose) 
		else:

			G = NNGraph(node_list = trees, dissimilarity = lambda x,y : dmins[x,y], verbose = self.verbose) 
...
class Qtree:
	def __init__(self, children = [], leave= False, parent = None, depth=0, verbose = 0):
...
class NNGraph:
	def __init__(self, node_list, dissimilarity, verbose=0):
		...
		self.node_ids = [i for i in range(len(node_list))] # internal enumeration of the elements
		self.weights =  lambda i,j : dissimilarity(node_list[i],node_list[j]) # function
		self.components = []
	...
	def nearest_neighbours(self, node_id):
		...
		nns = []  # current arg min
		min_dist =  self.weights(node_id,(node_id+1)%self.N) # nearest neighbour

		for i in self.node_ids: # nearest neight
			if i != node_id: 
				dist_i = self.weights(i, node_id)
				if dist_i < min_dist: # if dist of current is less than best known
	...
	def get_DFS_order(self):
		...
		for v in self.borders:
			...
			if v not in visited:
				...
				tree = Qtree(children=[self.nodes[i] for i in component],depth = pre_depth+1, verbose=self.verbose)
				...
				if self.verbose > 2:
					...
					print("borders")
					print(tree.left_borders(), tree.right_borders(), tree.borders())
